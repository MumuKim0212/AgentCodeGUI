...
- 🔑 **API 키 불필요** — 이 PC의 Claude Code 로그인(OAuth, `~/.claude`)을 그대로 재사용
...
- `/ask` — 본 대화를 오염시키지 않는 일회용 사이드 질문 창
- 이미지 첨부(붙여넣기·드래그·선택기) + 라이트박스 뷰어
...
- **파일 뷰어**: 구문 강조 + LSP 코드 인텔리전스(호버 타입 정보 · `F12`/`Ctrl+클릭` 정의 이동 ·
  ...
  이미지 미리보기(png/jpg/svg…, 체커보드·핏·줌), 마크다운 렌더 보기
